...
func StandardWorker(threadId int, ms types.Microservice, redisClient *redis.Client, rtsClient *redistimeseries.Client, ctx context.Context) {
	if ms.BlockMS == 0 {
		ms.BlockMS = 10
	}
	if ms.BatchSize == 0 {
	...
	log.Printf("Starting worker: ThreadId: %d : %+v", threadId, ms)
	...
	for {
		res, _ := redisClient.XReadGroup(ctx, &redis.XReadGroupArgs{
			...
			Consumer: fmt.Sprintf("Consumer-%s-%d", ms.Input, threadId),
